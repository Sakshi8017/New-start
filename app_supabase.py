...
def update_student_profile(usn):
    ...
    success = db.update_student_profile(usn, skills, interests, bio)
    if success:
...
def load_students():
    """Load students from CSV into Supabase (run once)"""
    success = db.load_students_from_csv()
    if success:
        return jsonify({'success': True, 'message': 'Students loaded successfully'})
...
def on_send_message(data):
    ...
    if message_id:
        ...
        emit('new_message', {
            ...
            'message': message,
            'created_at': str(db.datetime.now())
        }, room=f'group_{group_id}')
